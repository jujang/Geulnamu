...
class _SettingsScreenState extends State<SettingsScreen> {
    ...
    try {
      ...
      if (accessToken != null && accessToken.isNotEmpty) {
        ...
        try {
          ...
          debugPrint(
            '✅ [SettingsScreen] 백엔드에서 푸시 설정 로드 완료: $_meetingNotificationEnabled',
          );
        } catch (e) {
          // 백엔드 조회 실패 시 로컬 값 사용 (폴백)
          debugPrint('⚠️ [SettingsScreen] 백엔드 조회 실패, 로컬 값 사용: $e');
          _meetingNotificationEnabled = await _settingsService
  ...
  Widget _buildSettingsContent(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment:
            CrossAxisAlignment.stretch, // 🎯 start → stretch로 변경 (가로 전체 사용)
        children: [
          // 🎨 외관 설정 섹션
